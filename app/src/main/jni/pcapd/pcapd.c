...
static int init_pcapd_capture(pcapd_runtime_t *rt, pcapd_conf_t *conf) {
    ...
    if (rt->inet_iface) {
        rt->nlroute_sock = nl_route_socket(
                RTMGRP_IPV4_ROUTE | RTMGRP_IPV4_IFADDR | RTMGRP_IPV4_RULE |
...
static pcapd_rv
open_interface(pcapd_iface_t *iface, pcapd_runtime_t *rt, const char *ifname, int ifid) {
...
static void check_inet_interface(pcapd_runtime_t *rt) {
    ...
    log_i("Internet interface changed [%d -> %d], (re)starting capture", rt->inet_iface->ifidx,
          ri.ifidx);
...
static int handle_nl_message(pcapd_runtime_t *rt) {
    ...
    for (struct nlmsghdr *nh = (struct nlmsghdr *) rt->nlbuf; NLMSG_OK(nh, len); nh = NLMSG_NEXT(nh,
                                                                                                 len)) {
        uint8_t do_break = 0;
            ...
            case RTM_NEWADDR:
                if ((iface = iface_by_ifidx(rt,
                    ...
                    if (get_iface_ip(iface->name, &iface->ip, &netmask) < 0)
                        log_i("Could not get interface \"%s\" IP[%d]: %s", iface->name, errno,
                              strerror(errno));

                    if (get_iface_ip6(iface->name, &iface->ip6) < 0) {
                        log_i("Could not get interface \"%s\" IPv6[%d]: %s", iface->name, errno,
                              strerror(errno));
                        memset(&iface->ip6, 0, sizeof(iface->ip6));
                    }
                }
                ...
                if (uid == -2) {
                    if ((rt->nldiag_sock > 0) && (zpkt.tuple.ipproto != IPPROTO_ICMP)) {
                        ...
                        if ((uid < 0) && (uid != UID_UNKNOWN)) {
                            log_e("nl_get_uid failed with error %d [%d]: %s", uid, errno,
                                  strerror(errno));
                            close(rt->nldiag_sock);
                            rt->nldiag_sock = -1;
