...
static void
update_connection_status(pcapdroid_t *pd, pcap_conn_t *conn, zdtun_pkt_t *pkt, uint8_t dir) {
...
static bool handle_packet(pcapdroid_t *pd, pcapd_hdr_t *hdr, const char *buffer, int ipoffset) {
    ...
    uint8_t is_tx = (hdr->flags &
                     PCAPD_FLAG_TX); // NOTE: the direction uses an heuristic so it may be wrong
    ...
    if (pd->pcap_file_capture && (hdr->uid == UID_UNKNOWN)) {
        ...
        if (non_ip_overhead >= sizeof(pcapdroid_trailer_t)) {
            const struct pcapdroid_trailer *trailer =
                    (const struct pcapdroid_trailer *) (buffer + hdr->len -
                                                        sizeof(pcapdroid_trailer_t));
...
static void purge_expired_connections(pcapdroid_t *pd, uint8_t purge_all) {
    ...
    HASH_ITER(hh, pd->pcap.connections, conn, tmp) {
        ...
        switch (conn->tuple.ipproto) {
            case IPPROTO_TCP:
                timeout = (conn->data->status >= CONN_STATUS_CLOSED) ? (TCP_CLOSED_TIMEOUT_SEC *
                                                                        1000) : (TCP_TIMEOUT_SEC *
                                                                                 1000);
                break;
            case IPPROTO_UDP:
        ...
        if (purge_all || (pd->now_ms >= (conn->data->pcap.last_update_ms + timeout))) {
            ...
            if ((conn->data->update_type == 0) ||
                (pd_notify_connection_update(pd, &conn->tuple, conn->data) < 0)) {
...
int run_libpcap(pcapdroid_t *pd) {
    ...
    pd->pcap.app_filter_uids_size = getIntArrayPref(pd->env, pd->capture_service,
                                                    "getAppFilterUids", &pd->pcap.app_filter_uids);
    pd->pcap.as_root = !pd->pcap_file_capture;
    pd->pcap.bpf = getStringPref(pd, "getPcapDumperBpf", bpf, sizeof(bpf));
    pd->pcap.capture_interface = getStringPref(pd, "getCaptureInterface", capture_interface,
                                               sizeof(capture_interface));
#endif
    ...
    if (pd->tls_decryption.enabled) {
        ...
        if (pd->pcap.app_filter_uids_size > 0) {
            for (int i = 0; i < pd->pcap.app_filter_uids_size; i++) {
                ...
                if (uid >= 0) {
                    if (run_shell_cmd("iptables", get_mitm_redirection_args(pd, args, uid, true),
                                      true, true) != 0)
                        goto cleanup;
            ...
            if (run_shell_cmd("iptables", get_mitm_redirection_args(pd, args, -1, true), true,
                              true) != 0)
                goto cleanup;
    ...
    if (iptables_cleanup) {
        ...
        if (pd->pcap.app_filter_uids_size > 0) {
            for (int i = 0; i < pd->pcap.app_filter_uids_size; i++) {
                ...
                if (uid >= 0)
                    run_shell_cmd("iptables", get_mitm_redirection_args(pd, args, uid, false), true,
                                  false);
            }
        } else
