...
uint16_t pd_ndpi2proto(ndpi_protocol proto) {
    ...
    uint16_t l7proto = ((proto.master_protocol != NDPI_PROTOCOL_UNKNOWN) ? proto.master_protocol
                                                                         : proto.app_protocol);
...
static int
notif_connection(pcapdroid_t *pd, conn_array_t *arr, const zdtun_5tuple_t *tuple, pd_conn_t *data) {
...
static void
check_blacklisted_domain(pcapdroid_t *pd, pd_conn_t *data, const zdtun_5tuple_t *tuple) {
    if (data->info && data->info[0]) {
        if (pd->malware_detection.bl && !data->blacklisted_domain && !data->whitelisted_app) {
            ...
            if (blacklisted) {
                ...
                if (pd->malware_detection.whitelist &&
                    blacklist_match_domain(pd->malware_detection.whitelist, data->info))
        ...
        if (pd->firewall.enabled && pd->firewall.bl && !data->to_block) {
            ...
            if (data->to_block) {
                ...
                get_appname_by_uid(pd, data->uid, appbuf, sizeof(appbuf));
                log_d("Blocked domain [%s]: %s [%s]", data->info,
                      zdtun_5tuple2str(tuple, buf, sizeof(buf)), appbuf);
            }
        }
    }
...
static void
check_whitelist_mode_block(pcapdroid_t *pd, const zdtun_5tuple_t *tuple, pd_conn_t *data) {
    ...
    if (pd->firewall.enabled && pd->firewall.wl_enabled && pd->firewall.wl && !data->to_block &&
        ...
        (!is_dns ||
         ((data->uid != UID_NETD) && (data->uid != UID_PHONE) && (data->uid != UID_UNKNOWN))))
            ...
            for (int i = 0; i < pd->new_conns.cur_items; i++) {
                ...
                if ((conn->data->uid == UID_NETD)
                    ...
                    if (!conn->data->to_block && pd->firewall.enabled && pd->firewall.bl && (
                            ...
                            (pd->firewall.wl_enabled && pd->firewall.wl &&
                             !blacklist_match_uid(pd->firewall.wl, conn->data->uid))))
    ...
    if (pd->malware_detection.bl) {
        if (!data->whitelisted_app) {
            ...
            if (blacklisted) {
                ...
                if (pd->malware_detection.whitelist &&
                    blacklist_match_ip(pd->malware_detection.whitelist, &dst_ip, tuple->ipver))
                    log_d("Whitelisted dst ip: %s [%s]", zdtun_5tuple2str(tuple, buf, sizeof(buf)),
                          appbuf);
                else {
                    log_w("Blacklisted dst ip: %s [%s]", zdtun_5tuple2str(tuple, buf, sizeof(buf)),
                          appbuf);
...
static void process_payload(pcapdroid_t *pd, pkt_context_t *pctx) {
    ...
    if ((pd->payload_mode == PAYLOAD_MODE_NONE) ||
        ...
        (pd->tls_decryption.enabled &&
         data->proxied)) // NOTE: when performing TLS decryption, TCP connections data is handled by the MitmReceiver
        return;

    if ((pd->payload_mode != PAYLOAD_MODE_MINIMAL) || !data->has_payload[pctx->is_tx]) {
        int to_dump = pkt->l7_len;

        if ((pd->payload_mode == PAYLOAD_MODE_MINIMAL) &&
            (pkt->l7_len > MINIMAL_PAYLOAD_MAX_DIRECTION_SIZE)) {
...
static void perform_dpi(pcapdroid_t *pd, pkt_context_t *pctx) {
    ...
    struct ndpi_proto n_proto = ndpi_detection_process_packet(pd->ndpi, data->ndpi_flow,
                                                              (const u_char *) pkt->buf,
    ...
    if ((data->l7proto == NDPI_PROTOCOL_DNS)
        ...
        && ((netd_resolve_waiting > 0) ||
            ((next_connections_dump - NETD_RESOLVE_DELAY_MS) < pd->now_ms))) {
        if (netd_resolve_waiting == 0) {
    ...
    for (int i = 0; i < pd->malware_detection.num_bls; i++) {
        ...
        if (blacklist_load_file(bl, get_file_path(pd, subpath), blinfo->type, &stats) == 0) {
            ...
            if (status_arr->size >= status_arr->cur_items) {
                ...
                status_arr->items = pd_realloc(status_arr->items,
                                               status_arr->size * sizeof(bl_status_t));
                if (!status_arr->items) {
...
static int check_blocked_conn_cb(pcapdroid_t *pd, const zdtun_5tuple_t *tuple, pd_conn_t *data) {
    ...
    data->to_block = (data->blacklisted_internal || data->blacklisted_ip ||
...
static int
check_blacklisted_conn_cb(pcapdroid_t *pd, const zdtun_5tuple_t *tuple, pd_conn_t *data) {
