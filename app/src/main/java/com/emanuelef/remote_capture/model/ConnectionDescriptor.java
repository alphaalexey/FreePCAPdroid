...
public class ConnectionDescriptor {
    ...
    public static String getStatusLabel(Status status, Context ctx) {
        ...
        switch (status) {
            case STATUS_ACTIVE:
                resid = R.string.conn_status_active;
                break;
            case STATUS_CLOSED:
    ...
    public static String getDecryptionStatusLabel(DecryptionStatus status, Context ctx) {
        ...
        switch (status) {
            case CLEARTEXT:
                resid = R.string.not_encrypted;
                break;
            case NOT_DECRYPTABLE:
                resid = R.string.not_decryptable;
                break;
            case DECRYPTED:
                resid = R.string.decrypted;
                break;
            case ENCRYPTED:
                resid = R.string.status_encrypted;
                break;
            case WAITING_DATA:
                resid = R.string.waiting_application_data;
                break;
            default:
                resid = R.string.error;
        }
    ...
    public boolean isPortMappingApplied() {
        return port_mapping_applied;
    }

    public boolean isNotDecryptable() {
        return !decryption_ignored && (encrypted_payload || !mitm_decrypt);
    }

    public boolean isDecrypted() {
        return !decryption_ignored && !isNotDecryptable() && (getNumPayloadChunks() > 0);
    }

    public boolean isCleartext() {
        return !encrypted_payload && !encrypted_l7;
    }

    public synchronized int getNumPayloadChunks() {
        return payload_chunks.size();
    }
